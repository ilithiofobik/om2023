...
# bierzemy zajęcia z zadanego kursu w danej grupie
var x {Groups, Courses} binary;
# trenujemy w danej grupie treningowej
...
# s.t. no_preference_less_that_5 {g in Groups, c in Courses : Preference[g,c] < 5 } : x[g,c] = 0;

# mamy nie więcej niż cztery godziny zajęć dziennie
s.t. max_day_length {d in 1..5} : 
    sum {g in Groups, c in Courses : Day[g,c] = d} (EndingTime[g,c] - StartingTime[g,c]) * x[g,c] <= 4;
# bierzemy dokładnie jedną grupę z danego kursu
s.t. one_group_per_course {c in Courses} : 
    sum {g in Groups} x[g,c] = 1;
# ćwiczenia na siebie nie nachodzą
s.t. no_overlap {g1 in Groups, g2 in Groups, c1 in Courses, c2 in Courses : 
    ...
    StartingTime[g2, c2] <= EndingTime[g1, c1] } : 
        x[g1,c1] + x[g2,c2] <= 1;
# trening nie zaczyna się w trakcie ćwiczeń
s.t. no_PE_overlap {g1 in Groups, c1 in Courses, p in PEGroups : 
    ...
    PEStartingTime[p]    <= EndingTime[g1, c1] } : 
        x[g1,c1] + y[p] <= 1;
# ćwiczenia nie zaczynają się w trakcie treningu
s.t. no_PE_rev_overlap {g1 in Groups, c1 in Courses, p in PEGroups : 
    ...
    StartingTime[g1,c1] <= PEEndingTime[p] } : 
        x[g1,c1] + y[p] <= 1;
# mamy co najmniej godzinę na lunch w przedziale 12 do 14
...
# mamy co najmniej jeden trening w tygodniu
